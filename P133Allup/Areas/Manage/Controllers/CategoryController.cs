...
namespace P133Allup.Areas.Manage.Controllers
{
    ...
    public class CategoryController : Controller
    {
        ...
        public async Task<IActionResult> Create(Category category)
        {
            ...
            if (category.IsMain)
            {
                foreach (var files in category.Files)
                {
                    if (files == null)
                    {
                    ...
                    if (!files.CheckFileContentType("image/jpeg"))
                    {
                        ModelState.AddModelError("File", "Fayl Tipi Duz Deyil");
                        return View(category);
                    }

                    if (!files.CheckFileLength(300))
                    {
                        ModelState.AddModelError("File", "Fayl Olcusu Maksimum 30 kb Ola Biler");
                        return View(category);
                    }

                    category.Image = await files.CraeteFileAsync(_env, "assets", "images");
                ...
                foreach (var file in category.Files)
                {
                    if (dbCategory.IsMain)
                    {
                        if (file != null)
                        {
                            ...
                            dbCategory.Image = await file.CraeteFileAsync(_env, "assets", "images");
                        }
                    }
                    else
                    {
